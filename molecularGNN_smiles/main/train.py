...
class Tester(object):
    ...
    def test_regressor(self, dataset, save_predictions=False):
        N = len(dataset)
        ...
        for i in range(0, N, batch_test):
            ...
            predicted_values, correct_values = self.model.forward_regressor(
                                               data_batch, train=False)
            all_predicted_values = np.concatenate((all_predicted_values,predicted_values))
            all_correct_values = np.concatenate((all_correct_values, correct_values))
            SAE += sum(np.abs(predicted_values-correct_values))
        MAE = SAE / N  # mean absolute error.
        if save_predictions:
            return all_predicted_values, all_correct_values
        return MAE
...
if __name__ == "__main__":
    ...
    file_result = '../output/result--' + str((radius, dim, layer_hidden, layer_output,
     batch_train, batch_test, decay_interval,
     iteration)) + '.txt'
    if task == 'classification':
    ...
    for epoch in range(iteration):
        ...
        print(result)
    torch.save(model,'../output/model--' + str(task) + "--" + str(iteration) + "iters" + '.pck')